    ...
    void operator()(CreateEvent& create_events){
        ...
        for(int i = 0; i < height; i++){
            ...
            for(int j = tmp; j < length; j++){
                ...
                std::mt19937 rng(dev());
                std::uniform_int_distribution<std::mt19937::result_type> dist(1, length+height-(length+height)%arm);
                if (dist(rng) == 4){
                    if((i != height - 1 || j != length - 1) && (i != height - 1 || j != 0)){
                        (*array)[i][j].set_players_events(create_events.create_Event_Arm());
